...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const radios = document.querySelectorAll(`input[name="${BRANDING_GROUP_NAME}"], input[name="${DEVELOPMENT_GROUP_NAME}"]`);
    if (checkboxes.length === 0 || !totalValueDiv || !hiddenTotalInput)
        return;
    let sum = 0;
    ...
    for (const select of selects) {
        ...
        if (!isNaN(previousSelectedValue))
    ...
    for (const radio of radios) {
        ...
        if (isNaN(amountToBeAdded) && checked)
            sum += amountToBeAdded;
