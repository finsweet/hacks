...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const BUTTON_SELECTOR = '[fs-hacks-element="hack26-button"]';
  const MOBILE_TEXT_SELECTOR = '[fs-hacks-element="hack26-text-mobile"]';
  const DESKTOP_TEXT_SELECTOR = '[fs-hacks-element="hack26-text-desktop"]';
  const runTestButton = document.querySelector(BUTTON_SELECTOR);
  const mobileText = document.querySelector<HTMLHeadingElement>(MOBILE_TEXT_SELECTOR);
  const desktopText = document.querySelector<HTMLHeadingElement>(DESKTOP_TEXT_SELECTOR);
  // early return
