...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const androidHeading = document.querySelector<HTMLHeadingElement>(ANDROID_SELECTOR);
  const iosHeading = document.querySelector<HTMLHeadingElement>(IOS_SELECTOR);
  const chromeHeading = document.querySelector<HTMLHeadingElement>(CHROME_SELECTOR);
...
const testBrowser = function (regexp: RegExp) {
