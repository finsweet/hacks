document.addEventListener('DOMContentLoaded', () => {
  ...
  const counterDivs = document.querySelectorAll<HTMLDivElement>(COUNTER_DIV_SELECTOR);
  ...
  counterDivs.forEach((counterDiv) => {
    ...
    const incrementButton = counterDiv.querySelector<HTMLButtonElement>(FORM_BUTTON_UP_SELECTOR);
    const decrementButton = counterDiv.querySelector<HTMLButtonElement>(FORM_BUTTON_DOWN_SELECTOR);
