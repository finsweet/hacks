...
export const fetchTsCode = async () => {
  if (!hackName) return;
  const url = `https://cdn.jsdelivr.net/gh/finsweet/hacks@d3f4ff72986753fd24e67a7100952f2913f9ee2c/src/webflow-hacks/${hackName}/${hackName}.ts`;
...
export const fetchJsCode = async () => {
  const url = `https://cdn.jsdelivr.net/gh/finsweet/hacks@d3f4ff72986753fd24e67a7100952f2913f9ee2c/src/webflow-hacks/${hackName}/${hackName}.js`;
  const code = await fetchCode(url);
  if (!code) return;
  const { formattedCode } = code;
...
const fetchComponentJSON = async () => {
  ...
  const url = `https://cdn.jsdelivr.net/gh/finsweet/hacks@d3f4ff72986753fd24e67a7100952f2913f9ee2c/src/webflow-hacks/${hackName}/${hackName}.json`;
