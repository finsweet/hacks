...
export const fetchTsCode = async () => {
  if (!hackName) return;
  const url = `https://cdn.jsdelivr.net/gh/finsweet/hacks@94e79d94c51aedc7e9ea357687ffa3a10d540a8c/src/webflow-hacks/${hackName}/${hackName}.ts`;
...
export const fetchJsCode = async () => {
  const url = `https://cdn.jsdelivr.net/gh/finsweet/hacks@94e79d94c51aedc7e9ea357687ffa3a10d540a8c/src/webflow-hacks/${hackName}/${hackName}.js`;
  const code = await fetchCode(url);
  if (!code) return;
  const { formattedCode } = code;
...
const fetchComponentJSON = async () => {
  ...
  const url = `https://cdn.jsdelivr.net/gh/finsweet/hacks@94e79d94c51aedc7e9ea357687ffa3a10d540a8c/src/webflow-hacks/${hackName}/${hackName}.json`;
